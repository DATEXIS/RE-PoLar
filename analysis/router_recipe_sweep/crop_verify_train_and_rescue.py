...
def main(argv=None) -> None:
    ...
    ap.add_argument("--out-data", required=True, help="where to write the crop-verified data variant")
    ...
    ap.add_argument("--prompt-style", default="paper_minimal_fewshot",
    ...
    from re_polar.router.train import (
        program_from_layer_path, build_examples, encode_examples, train,
        save_checkpoint, _resolve_device,
    )
    ...
    print(f"Loading {cfg['model_id']} + executor for crop-verification and training "
    ...
    for i, s in enumerate(train_samples):
        ...
        if (i + 1) % 100 == 0:
            print(f"  {i + 1}/{len(train_samples)} samples processed | "
                  f"pairs so far: {n_pairs} (kept {n_kept}, dropped {n_dropped})", flush=True)

    print(f"\nCrop-verification done: {n_pairs} (sample,path) pairs with times!=2 checked", flush=True)
    print(f"  kept (cropped-to-2x version verified CORRECT): {n_kept}/{n_pairs} "
          f"= {n_kept/max(1,n_pairs):.4f}", flush=True)
    print(f"  dropped (cropped-to-2x version WRONG or no gt_ans): {n_dropped}/{n_pairs} "
          f"= {n_dropped/max(1,n_pairs):.4f}", flush=True)
    ...
    build_kwargs = dict(max_paths_per_sample=50, reweight_original_path=True,
                        original_path_weight=0.30, cap_keep=True,
    ...
    val_program_data = {"questions": vq_cache, "token_hiddens": token_hiddens, "valid_sets": valid_sets} if vq_cache else None
    select_by = "val_cache_acc_at1" if val_program_data is not None else "val_loss"

    router = fresh_router()
    res = train(router, examples, val_examples=val_examples, epochs=10, lr=3e-4,
                batch_size=32, lr_scheduler="cosine", warmup_steps=10,
                device=device, seed=args.seed, move_encodings_to_device=True,
                select_by=select_by, val_program_data=val_program_data, val_topk=5)
    ...
    identity_wrong = [s for s in test if s.get("gt_ans") and
                       identity not in {tuple(p) for p in (s.get("final_valid_transitions") or []) if p}]
    ...
    print("\n=== SUMMARY ===", flush=True)
    print(f"Crop-verification: {n_kept}/{n_pairs} = {n_kept/max(1,n_pairs):.4f} of times!=2 paths "
          f"verified correct after cropping to 2x", flush=True)

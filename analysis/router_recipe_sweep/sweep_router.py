...
def main(argv=None):
    ...
    p = argparse.ArgumentParser(
        description="Paper-faithful router sweep (val-loss selection, per-difficulty)."
    )
    p.add_argument(
        "--samples", required=True, nargs="+", help="merged_mcts_samples.json (one per diff)"
    )
    p.add_argument("--model", default="qwen3_8b")
    p.add_argument("--out", required=True, help="checkpoint path (per-difficulty appends _diffK)")
    p.add_argument(
        "--select-metric",
        choices=["val_loss", "val_passk", "val_cache_acc_at1"],
        default="val_loss",
    ...
    original_path_weights = (
        [float(x) for x in args.original_path_weights.split(",") if x.strip()]
        if args.original_path_weights
        else [args.original_path_weight]
    )
    ...
    print(
        f"selection metric: {args.select_metric}  |  grouping: "
    ...
    print(
        f"original-path handling: {'DROP' if args.drop_original_path else 'reweight-0.30'}"
        f"{' (IGNORED -- label-mode=shortest)' if args.label_mode == 'shortest' else ''}",
        flush=True,
    )
    if args.original_path_weights:
        print(
            f"original-path-weight SWEEP: {original_path_weights} (outermost axis, "
            f"examples rebuilt+re-encoded per value)",
            flush=True,
        )
    if any(al > 0.0 for al in anti_original_lambdas):
        print(
            f"anti-original-lambda SWEEP: {anti_original_lambdas} (inner axis alongside "
            f"lenpref_beta, no re-encode needed)",
            flush=True,
        )
    ...
    for gname, files in groups:
        ...
        for opw in original_path_weights:
            ...
            if not examples or not val_examples:
                print(
                    f"  SKIP {gname} opw={opw:.3g}: no examples (train {len(examples)}, val {len(val_examples)})",
                    flush=True,
                )
                continue
            ...
            if args.select_metric == "val_cache_acc_at1":
                ...
                for s in val_samples:
                    ...
                    vs = {
                        ...
                        for p in (s.get("final_valid_transitions") or [])
                        if p
                    }
                ...
                if vq_cache:
                    ...
                    print(
                        f"  WARNING {gname} opw={opw:.3g}: no val question has a non-empty "
            ...
            print(
                f"  train examples {len(examples)} | val examples {len(val_examples)} | "
                f"winner-eval questions {len(val_questions)} | original_path_weight={opw:.3g}",
                flush=True,
            )
            ...
            # ---- grid ----
            print(
                f"  {'opw':>6} {'lr':>8} {'batch':>6} {'epochs':>7} {'beta':>6} {'anti_lam':>8} {'val_loss':>9} {'sel_score':>10} {'nonident':>9} {'best?':>6}",
                flush=True,
            )
                        ...
                        for beta in lenpref_betas:
                            for anti_lambda in anti_original_lambdas:
                                ...
                                if inner_select_by == "val_cache_acc_at1":
                                    ...
                                    score = (
                                        res.best_metric
                                        if res.best_metric is not None
                                        else float("-inf")
                                    )
                                    ...
                                    score = _mean(
                                        grade_router_topk(tk, val_questions, val_gt, reward_fn)[0]
                                    )
                                ...
                                router.eval()
                                cfg_tk1 = predict_programs_topk(
                                    router, val_questions, k=1, batch_size=args.gen_batch_size
                                )
                                cfg_nonident = _mean(
                                    [
                                        0.0 if c[0].to_layer_path() == list(range(D)) else 1.0
                                        for c in cfg_tk1
                                    ]
                                )
                                is_best = (best is None) or (score > best["score"])
                                print(
                                    f"  {opw:>6.3g} {lr:>8.0e} {batch:>6} {epochs:>7} {beta:>6.3g} {anti_lambda:>8.3g} {cfg_vloss:>9.4f} {score:>10.4f} {cfg_nonident:>8.1%} {'  <=' if is_best else '':>6}",
                                    flush=True,
                                )
                                grid.append(
                                    {
                                        "original_path_weight": opw,
                                        "lr": lr,
                                        "batch": batch,
                                        "epochs": epochs,
                                        "lenpref_beta": beta,
                                        ...
                                        "val_loss": cfg_vloss,
                                        "sel_score": score,
                                        "nonidentity_rate_generation_free": cfg_nonident,
                                    }
                                )
                                cfg_record = {
                                    "score": score,
                                ...
                                if (
                                    ...
                                    or score > best_per_beta[beta]["score"]
                                ):
                                    best_per_beta[beta] = cfg_record
                                if opw not in best_per_opw or score > best_per_opw[opw]["score"]:
                                    best_per_opw[opw] = cfg_record
                                if (
                                    anti_lambda not in best_per_antilambda
                                    or score > best_per_antilambda[anti_lambda]["score"]
                                ):
        ...
        winner.eval()
        tk = predict_programs_topk(
            winner, val_questions, k=args.val_topk, batch_size=args.gen_batch_size
        )
        ...
        save_checkpoint(
            ...
            meta={
                ...
                "group": gname,
                "original_path_weight": best["original_path_weight"],
                "lr": best["lr"],
                "epochs": best["epochs"],
                "batch_size": best["batch"],
                "lenpref_beta": best["lenpref_beta"],
                "anti_original_lambda": best["anti_original_lambda"],
        ...
        print(
            f"  WINNER {gname}: original_path_weight={best['original_path_weight']:.3g} lr={best['lr']:.0e} batch={best['batch']} epochs={best['epochs']} "
            ...
            f"| router_p{args.val_topk} {win_pk:.4f} | nonident {nonident:.1%}  -> {out_path}",
            flush=True,
        )
        ...
        for beta in lenpref_betas:
            ...
            if cfg is best:
                ...
                b_tk = predict_programs_topk(
                    b_router, val_questions, k=args.val_topk, batch_size=args.gen_batch_size
                )
        ...
        for opw in original_path_weights:
            ...
            if cfg is best:
                ...
                o_router.to(device)
                o_router.eval()
                o_tk = predict_programs_topk(
                    o_router, val_questions, k=args.val_topk, batch_size=args.gen_batch_size
                )
                o_r_at_k, o_r_at_1, _ = grade_router_topk(o_tk, val_questions, val_gt, reward_fn)
                o_p1, o_pk = _mean(o_r_at_1), _mean(o_r_at_k)
                o_nonident = _mean(
                    [0.0 if c[0].to_layer_path() == list(range(D)) else 1.0 for c in o_tk]
                )
                o_router.to("cpu")
            opw_curve.append(
                {
                    "original_path_weight": opw,
                    "lenpref_beta": cfg["lenpref_beta"],
                    ...
                    "val_loss": cfg["val_loss"],
                    "val_p1": o_p1,
                    f"val_p{args.val_topk}": o_pk,
                    "nonidentity_rate": o_nonident,
                }
            )
        group_summaries[-1]["opw_curve"] = opw_curve
        print(
            f"  original-path-weight curve {gname} (best lr/bs/ep/beta PER weight, base_p1 {base_p1:.4f}):",
            flush=True,
        )
        print(
            f"    {'opw':>6} {'val_loss':>9} {'router_p1':>10} {'Δp1':>7} {'nonident':>9}",
            flush=True,
        )
        for c in opw_curve:
            print(
                f"    {c['original_path_weight']:>6.3g} {c['val_loss']:>9.4f} {c['val_p1']:>10.4f} "
                f"{c['val_p1'] - base_p1:>+7.4f} {c['nonidentity_rate']:>8.1%}",
                flush=True,
            )
        ...
        for anti_lambda in anti_original_lambdas:
            ...
            if cfg is best:
                ...
                a_router.to(device)
                a_router.eval()
                a_tk = predict_programs_topk(
                    a_router, val_questions, k=args.val_topk, batch_size=args.gen_batch_size
                )
                a_r_at_k, a_r_at_1, _ = grade_router_topk(a_tk, val_questions, val_gt, reward_fn)
                a_p1, a_pk = _mean(a_r_at_1), _mean(a_r_at_k)
                a_nonident = _mean(
                    [0.0 if c[0].to_layer_path() == list(range(D)) else 1.0 for c in a_tk]
                )
                a_router.to("cpu")
            antilambda_curve.append(
                {
                    "anti_original_lambda": anti_lambda,
                    "lenpref_beta": cfg["lenpref_beta"],
                    "original_path_weight": cfg["original_path_weight"],
                    ...
                    "val_loss": cfg["val_loss"],
                    "val_p1": a_p1,
                    f"val_p{args.val_topk}": a_pk,
                    "nonidentity_rate": a_nonident,
                }
            )
        group_summaries[-1]["anti_original_lambda_curve"] = antilambda_curve
        print(
            f"  anti-original-lambda curve {gname} (best lr/bs/ep/beta/opw PER lambda, base_p1 {base_p1:.4f}):",
            flush=True,
        )
        print(
            f"    {'anti_lam':>8} {'val_loss':>9} {'router_p1':>10} {'Δp1':>7} {'nonident':>9}",
            flush=True,
        )
        for c in antilambda_curve:
            print(
                f"    {c['anti_original_lambda']:>8.3g} {c['val_loss']:>9.4f} {c['val_p1']:>10.4f} "
                f"{c['val_p1'] - base_p1:>+7.4f} {c['nonidentity_rate']:>8.1%}",
                flush=True,
            )
    ...
    print("\n" + "=" * 72, flush=True)
    print(
        f"{'group':>7} {'opw':>6} {'lr':>8} {'bs':>5} {'ep':>4} {'beta':>6} {'anti_lam':>8} {'val_loss':>9} {'base_p1':>8} {'router_p1':>10} "
        f"{'Δp1':>7} {'router_pk':>10} {'nonident':>9}",
        flush=True,
    )
    for s in group_summaries:
        print(
            f"{s['group']:>7} {s['original_path_weight']:>6.3g} {s['lr']:>8.0e} {s['batch']:>5} {s['epochs']:>4} {s['lenpref_beta']:>6.3g} {s['anti_original_lambda']:>8.3g} {s['val_loss']:>9.4f} {s['base_val_p1']:>8.4f} "
            f"{s['val_p1']:>10.4f} {s['val_p1'] - s['base_val_p1']:>+7.4f} {s[f'val_p{args.val_topk}']:>10.4f} "
            f"{s['nonidentity_rate']:>8.1%}",
            flush=True,
        )
    ...
    out = {
        ...
        "original_path_weights": original_path_weights,
        "anti_original_lambdas": anti_original_lambdas,

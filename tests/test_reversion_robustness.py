...
from analysis.structural_analysis.reversion_robustness import aggregate, classify_reversions, load_by_query
...
def _repeat_path(start, length, times=2):
    return list(range(start)) + list(range(start, start + length)) * times + list(range(start + length, D))
...
def test_reversion_pair_no_break_when_keep_side_also_correct():
    ...
    records = [
        ...
        {"query_id": "q1", "path": [l for l in range(D) if not (4 <= l < 8) and not (8 <= l < 12)], "reward": 1.0},
...
def test_load_by_query_released_merged_mcts_samples(tmp_path):
    ...
    data = {"samples": [
        ...
        },
        {
            ...
            "question": "q3 text", "gt_ans": "3",
            "final_valid_transitions": [], "final_invalid_transitions": [],
            "initial_transition_metric": 0.0,
...
def test_load_by_query_both_formats_give_identical_classification(tmp_path):
    ...
    merged.write_text(json.dumps({"samples": [{
        "question": "q", "gt_ans": "1", "final_valid_transitions": [_skip_path(8, 4)],
        "final_invalid_transitions": [], "initial_transition_metric": 0.0,
        "sample_info": {"query_id": "q1"},
        "search_trajectory": [
            {"path": _skip_path(8, 4), "parent_path": _identity_path(), "reward": 1.0},
        ],

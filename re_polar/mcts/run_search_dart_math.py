...
def main():
    ...
    parser.add_argument("--max-repeat-times", type=int, default=2,
    ...
    parser.add_argument("--prompt-style", default="paper_minimal_fewshot",
    ...
    parser.add_argument("--replica-seq-len", type=int, default=1024,
    ...
    cache_path = (Path(args.cache_dir) / f"{args.model}_diff{args.difficulty}.jsonl"
    ...
    if args.offline_replay:
        ...
        fail_log = (str(Path(args.cache_dir) / f"grader_fails_{args.model}_diff{args.difficulty}.jsonl")
                    if args.cache_dir else None)
        ...
        answer_log = (str(Path(args.cache_dir) /
                          f"answers_{args.model}_diff{args.difficulty}.jsonl")
                      if args.log_answers else None)
        reward_kwargs = dict(max_new_tokens=args.max_new_tokens, batch_size=args.gen_batch_size,
                             difficulty=args.difficulty, fail_log_path=fail_log,
                             masked_batch_max_group_size=args.masked_batch_max_group_size,
                             masked_batch_buckets=args.masked_batch_buckets,
                             text_log_path=answer_log, prompt_style=args.prompt_style)
        ...
        if replicas_arg == "1":
            ...
            from re_polar.mcts.replica_pool import build_budgeted_replicas, build_generation_replicas
            trc = cfg.get("trust_remote_code", True)
            if replicas_arg == "auto":
                reward_fns, n, diag = build_budgeted_replicas(
                    cfg["model_id"], max_replicas=args.max_replicas,
                    batch_size=args.gen_batch_size, seq_len=args.replica_seq_len,
                    trust_remote_code=trc, reward_kwargs=reward_kwargs)
                print(f"replica pool: auto-sized to {n} replica(s), {diag}")
            else:
                reward_fns = build_generation_replicas(
                    cfg["model_id"], int(replicas_arg), trust_remote_code=trc,
    ...
    if args.offline_replay and str(args.replicas).strip().lower() != "1":
        raise SystemExit("--offline-replay has no model, so --replicas must be 1 (the replica "
                         "pool only parallelizes real GPU generation).")
    if args.offline_replay and args.max_repeat_times != 2:
        raise SystemExit("--offline-replay requires --max-repeat-times 2 (the cache only "
                         "covers the paper's 2x loops; deeper loops need a fresh search).")
    if args.offline_replay and per_tree_seed:
        raise SystemExit("--offline-replay replays an EXISTING shared-seed trajectory; the "
    ...
    if args.offline_replay and args.epsilon != 0.0:
        raise SystemExit("--offline-replay replays the EXISTING epsilon=0.0 trajectory; "
    ...
    if args.offline_replay and args.global_selection:
        raise SystemExit("--offline-replay only supports the hierarchical mode's trajectory -- "
    ...
    runner = MCTSRunner(inputs, num_layers=num_layers, reward_fn=reward_fn,
                        ...
                        reward_fns=reward_fns, epsilon=args.epsilon,
    ...
    write_merged_samples(Path(args.output_root), cfg["model_id"],
                         f"dart-math-diff-{args.difficulty}", samples)
    ...
    print(f"PILOT GATE (>=0.60 shorter-valid among solved): "

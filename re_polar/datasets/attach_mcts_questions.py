...
def attach(mcts_dir: Path, out_dir: Path, lookup: Dict[str, Tuple[str, str]]) -> None:
    files = sorted(mcts_dir.glob("*/dart-math-diff-*/merged_mcts_samples.json")) + \
        sorted(mcts_dir.glob("*/dart-math-diff-*/merged_mcts_samples.json.gz"))
    if not files:
        raise FileNotFoundError(f"No */dart-math-diff-*/merged_mcts_samples.json[.gz] under {mcts_dir}")
...
def main(argv=None) -> None:
    ...
    p.add_argument("--dart-math-dir", required=True,
                   help="Output dir of `re_polar.datasets.dart_math --include-gsm8k`")
    p.add_argument("--out-dir", required=True, help="Where to write the full merged_mcts_samples.json files")
    args = p.parse_args(argv)

    lookup = build_question_lookup(Path(args.dart_math_dir))
    print(f"Loaded {len(lookup)} query_id -> question/gt_ans entries from {args.dart_math_dir}")
